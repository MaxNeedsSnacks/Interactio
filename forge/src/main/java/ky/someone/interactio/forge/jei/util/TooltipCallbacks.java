package ky.someone.interactio.forge.jei.util;
...
public final class TooltipCallbacks {
    ...
    public static void returnChance(int idx, boolean input, List<Component> tooltip, List<Double> returnChances) {
        if (input && (idx >= 0) && (idx < returnChances.size())) {
            ...
            if (returnChance != 0) {
                tooltip.add(Utils.translate("interactio.recipe_info.return_chance", null, Utils.formatChance(returnChance, ChatFormatting.ITALIC)));
            }
        }
    }
    ...
    public static <T> void weightedOutput(boolean input, T stack, List<Component> tooltip, WeightedOutput<T> output, WeightedOutput.WeightedEntry<T> empty, boolean allowMultiple, Predicate<WeightedOutput.WeightedEntry<T>> equals) {
        if (!input) {
            if (!output.isSingle()) {
                ...
                if (match == empty) {
                    tooltip.clear();
                    tooltip.add(Utils.translate("interactio.recipe_info.weighted_output_empty", Style.EMPTY.withBold(true)));
                }

                tooltip.add(Utils.translate("interactio.recipe_info.weighted_output_chance", null, Utils.formatChance(output.getChance(match), ChatFormatting.ITALIC)));

                if (allowMultiple && output.rolls > 1) {
                    tooltip.add(Utils.translate("interactio.recipe_info.weighted_output_roll_count",
                            Style.EMPTY.withColor(ChatFormatting.GRAY),
                            output.unique ? Utils.translate("interactio.recipe_info.weighted_output_roll_unique", null, output.rolls)
                                    : output.rolls));
                }
            }
        }
