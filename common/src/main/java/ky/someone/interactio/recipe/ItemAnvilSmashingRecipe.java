...
import ky.someone.interactio.recipe.util.InWorldRecipe.DefaultInfo;
...
public final class ItemAnvilSmashingRecipe implements InWorldRecipe<List<ItemEntity>, BlockState, DefaultInfo> {
    ...
    public void craft(List<ItemEntity> entities, DefaultInfo info) {
        ...
        while (Utils.compareStacks(loopingEntities, used, inputs) && !anvilBroke) {
            ...
            if (rand.nextDouble() < damage) {
                Utils.sendParticle(new BlockParticleOption(ParticleTypes.BLOCK, level.getBlockState(pos)), level, Vec3.atCenterOf(pos), 25);
            ...
            Utils.sendParticle(ParticleTypes.END_ROD, level, Vec3.atCenterOf(pos));
