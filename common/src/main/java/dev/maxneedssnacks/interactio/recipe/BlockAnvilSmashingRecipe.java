...
public final class BlockAnvilSmashingRecipe implements InWorldRecipe<BlockPos, BlockState, InWorldRecipe.DefaultInfo> {
    ...
    public void craft(BlockPos anvilPos, DefaultInfo info) {
        ...
        if (rand.nextDouble() < damage) {
            sendParticle(new BlockParticleOption(ParticleTypes.BLOCK, level.getBlockState(anvilPos)), level, Vec3.atCenterOf(anvilPos), 25);
    ...
    private static class Serializer implements RecipeSerializer<BlockAnvilSmashingRecipe> {
        @Override
